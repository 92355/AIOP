...
export function SubscriptionSummary({ initialSubscriptions }: { initialSubscriptions: Subscription[] }) {
  ...
  return (
    <section className={`rounded-2xl border border-zinc-800 bg-zinc-900 shadow-soft ${isCompact ? "p-4" : "p-5"}`}>
      ...
      {isCompact ? null : <p className="mt-1 text-sm text-zinc-500">반복 지출을 상태별로 확인합니다.</p>}
      <div className={`mt-5 grid gap-3 ${isCompact ? "grid-cols-2" : "sm:grid-cols-4"}`}>
        <div className={`rounded-2xl bg-zinc-950/70 ${isCompact ? "hidden" : "p-4"}`}>
          ...
          <p className="mt-1 text-xl font-semibold text-zinc-50"><FlipNumber value={formatKRW(summary.total)} /></p>
        </div>
        <div className={`rounded-2xl bg-zinc-950/70 p-4 ${isCompact ? "hidden" : ""}`}>
          <p className="text-sm text-zinc-500">유지</p>
          <p className="mt-1 text-xl font-semibold text-emerald-300"><FlipNumber value={`${summary.keepCount}개`} /></p>
        </div>
        <div className="rounded-2xl bg-zinc-950/70 p-4">
          <p className="text-sm text-zinc-500">검토</p>
          <p className="mt-1 text-xl font-semibold text-zinc-100"><FlipNumber value={`${summary.reviewCount}개`} /></p>
        </div>
        <div className="rounded-2xl bg-zinc-950/70 p-4">
          <p className="text-sm text-zinc-500">해지</p>
          <p className="mt-1 text-xl font-semibold text-red-300"><FlipNumber value={`${summary.cancelCount}개`} /></p>
        </div>
      </div>
      <div className={`mt-4 flex flex-wrap gap-2 ${isCompact ? "max-h-16 overflow-hidden" : ""}`}>
        {items.length === 0 ? (
          <span className="rounded-2xl border border-zinc-800 px-3 py-2 text-sm text-zinc-500">아직 추가한 구독이 없습니다.</span>
        ) : null}
        {summary.actionItems.length === 0 && items.length > 0 ? (
          <span className="rounded-2xl border border-zinc-800 px-3 py-2 text-sm text-zinc-500">지금 검토할 구독이 없습니다.</span>
        ) : null}
        {summary.actionItems.map((item) => (
          <span key={item.id} className="rounded-full border border-zinc-800 px-3 py-2 text-sm text-zinc-300">{item.service}</span>
        ))}
      </div>
    </section>
  );
